...
class UpLoadImg extends React.Component {
	constructor(props) {
		super(props);
		console.log('____________________',props)
		this.state = {
			img: '',
			imageList: props.url||[]
		}
	...
	postImgToUploadImg = (base64) => {
		...
		//  http://test-piclib.cx580.com:9016/api/upload
		axios.post('https://inet-galleryapi.cx580.com/photo_gallery_api/photoUpload/uploadFileForMobileEnd', toQueryString(uploadimgData), {
			headers: {
			...
			.then(res => {
				...
				if (res.data.code == 0) {
					console.log(res.data.data.thumbnails)
					this.props.getImgUrl(this.props.type, res.data.data.url)
				} else {
					Message.destroy();
	...
	render() {
		...
		return (
			<div style={{ position: 'relative' }}>
				...
				/>
				{/* { (typeof this.props.url =='string'&&this.props.url) && } */}
